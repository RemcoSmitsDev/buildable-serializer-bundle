...
| `AbstractObjectNormalizer::MAX_DEPTH_HANDLER` | `max_depth` feature | An optional `callable` invoked when `ENABLE_MAX_DEPTH` is `true` and a property's depth limit has been reached. The callable receives `($attributeValue, $object, $attributeName, $format, $context)` — matching Symfony's contract exactly — and its return value is stored as the property value (even if `null`). When this key is absent and the depth limit is exceeded, the property is omitted from the output entirely. Has no effect when `ENABLE_MAX_DEPTH` is absent or `false`. |
