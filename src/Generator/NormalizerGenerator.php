...
final class NormalizerGenerator implements NormalizerGeneratorInterface
{
    ...
    private function writeClassmap(array $classmap): void
    {
        ...
        foreach ($classmap as $fqcn => $filePath) {
            $entries[] = sprintf('    %s => %s,', var_export($fqcn, true), var_export($filePath, true));
        }
    ...
    public function generate(ClassMetadata $metadata): string
    {
        ...
        $buf->line('final class ' . $normalizerClass . ' implements ' . implode(', ', $implements));
        $buf->line('{');
        $buf->indent();
        ...
        $buf->line('/** Priority in the Symfony Serializer normalizer chain (higher = earlier). */');
        $buf->line('public const NORMALIZER_PRIORITY = ' . self::DEFAULT_PRIORITY . ';');
     ...
     */
    private function writeNormalizeMethod(CodeBuffer $buf, ClassMetadata $metadata, bool $needsCircularRef): void
    {
        ...
        if ($hasGroups) {
            ...
            $buf->line('$groups = (array) ($context[AbstractNormalizer::GROUPS] ?? []);');
        }

        if ($hasSkipNull) {
            $buf->line('$skipNullValues = (bool) ($context[AbstractObjectNormalizer::SKIP_NULL_VALUES] ?? false);');
        }

        if ($hasNameConv) {
            $buf->line('$nameConverter = $context[AbstractNormalizer::NAME_CONVERTER] ?? null;');
        }
     ...
     */
    private function writeCircularReferenceGuard(CodeBuffer $buf, string $targetFqcn): void
    {
        ...
        $buf->blank();
        $buf->line('if (isset($context[\'circular_reference_limit_counters\'][$objectHash])) {');
        $buf->indent();
        $buf->line('$limit = (int) ($context[AbstractNormalizer::CIRCULAR_REFERENCE_LIMIT] ?? 1);');
        $buf->blank();
        $buf->line('if ($context[\'circular_reference_limit_counters\'][$objectHash] >= $limit) {');
        $buf->indent();
        $buf->line('if (isset($context[AbstractNormalizer::CIRCULAR_REFERENCE_HANDLER])) {');
        $buf->indent();
        $buf->line('return ($context[AbstractNormalizer::CIRCULAR_REFERENCE_HANDLER])($object, $format, $context);');
        $buf->outdent();
        $buf->line('}');
        $buf->blank();
        $buf->line('throw new CircularReferenceException(sprintf(');
        $buf->indent();
        $buf->line('\'A circular reference has been detected when serializing the object of class "%s" '
        . '(configured limit: %d).\',');
        ...
        $buf->line('}');
        $buf->blank();
        $buf->line('++$context[\'circular_reference_limit_counters\'][$objectHash];');
        $buf->outdent();
        $buf->line('} else {');
        $buf->indent();
        $buf->line('$context[\'circular_reference_limit_counters\'][$objectHash] = 1;');
        $buf->outdent();
        $buf->line('}');
    }
    ...
    private function writeNestedValue(
        ...
        if (!$needsNullCheck) {
            ...
            $buf->line(
                '$data[' . $keyExpr . '] = $this->normalizer->normalize(' . $rawValueExpr . ', $format, $context);',
            );
            return;
        }
    ...
    private function writeCollectionValue(
        ...
        $buf->line(
            '$data[' . $keyExpr . '] = $this->normalizer->normalize(' . $collectionRef . ', $format, $context);',
        );
     ...
     */
    private function writeSupportsNormalizationMethod(CodeBuffer $buf, string $targetFqcn): void
    {
     ...
     */
    private function writeGetSupportedTypesMethod(CodeBuffer $buf, string $targetFqcn): void
    {
        $shortName = $this->shortName($targetFqcn);

        $buf->line('/**');
        $buf->line(' * @return array<class-string|\'*\'|\'object\'|string, bool|null>');
        $buf->line(' */');
     ...
     *       : 'firstName'`
     */
    private function buildNameConverterKeyExpr(string $rawKey, string $ownerClass): string
    {
        return sprintf(
            '$nameConverter instanceof NameConverterInterface'
